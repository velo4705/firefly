...
class OutlineBorder extends OutlinedBorder {
  ...
  Path getInnerPath(Rect rect, {TextDirection? textDirection}) {
    return Path()..addRect(rect.deflate(side.strokeWidth / 2));
  }
  ...
  @override
  OutlineBorder lerp(covariant OutlineBorder a, covariant OutlineBorder b, double t) {
  ...
  @override
  OutlineBorder copyWith({BorderSide? side}) {
    return OutlineBorder(
      side: side ?? this.side,
    );
  }
...
class _DirectorySelectionPageState extends State<DirectorySelectionPage> {
  ...
  Future<void> _startScan() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        final directoryPath = _pathController.text;
        ...
        // Dispatch BLoC event
        context.read<LocalMusicBloc>().add(
          ScanLocalMusicEvent(directoryPath),
        );
        ...
        final provider = context.read<LocalMusicProvider>();
        provider.setScanning(false);
        provider.setError(_errorMessage!);
      }
    }
  }
